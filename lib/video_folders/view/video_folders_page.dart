...
class VideoFoldersView extends StatelessWidget {
            ...
            listener: (context, state) {
              ...
              if (state.copyStatus.status == VideoFoldersCopyStatus.copying) {
                if (_progressIndicatorDialog?.isShowing == true) {
                  ...
                  if (current > 0) {
                    String title = "正在导出视频文件夹, 请稍后...";

                    if (state.copyStatus.fileType == VideoFoldersFileType.video) {
                      title = "正在导出视频，请稍后...";
                    }
  ...
  void _openMenuForFolders(
    ...
    if (checkedFolders.length == 1) {
      ...
      copyTitle = "拷贝 ${checkedFolders.length} 项 到 电脑".adaptForOverflow();
    }
  ...
  void _tryToDeleteVideoFolders(
    ...
    CommonUtil.showConfirmDialog(
        pageContext,
        "确定删除这${checkedFolders.length}个项目吗？",
        "注意：删除的文件无法恢复",
        "取消",
        "删除", (context) {
  ...
  void _tryToDeleteVideos(
    ...
    CommonUtil.showConfirmDialog(
        pageContext,
        "确定删除这${checkedVideos.length}个项目吗？",
        "注意：删除的文件无法恢复",
        "取消",
        "删除", (context) {
      Navigator.of(context, rootNavigator: true).pop();
